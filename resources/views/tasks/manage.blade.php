...
<div class="content-card">
                    ...
                    @foreach($tasks as $index => $task)
                        <tr class="{{ $category === 'trash' ? 'text-muted' : '' }}">
                            ...
                            <td class="fw-medium">
                                ...
                                @if($category !== 'trash' && $task->status !== 'Completed' && $task->status !== 'Submitted')
                                    @php
                                        $now = now()->startOfDay();
                            ...
                            @elseif($category !== 'status')
                                <td>
                                    ...
                                    @endphp
                                    <span class="badge {{ $statusClass }} px-2 py-1" style="font-size: .75rem; font-weight: 600;">
                                        {{ $task->status }}
                                    </span>
                                </td>
                            @endif
                            ...
                            <td class="text-end">
                                ...
                                @else
                                    ...
                                    <button type="button" class="btn btn-sm btn-update btn-edit-task"
                                            title="Edit Task"
    ...
    <div class="d-flex align-items-center justify-content-between mt-4 flex-wrap gap-2">
        <p class="text-muted mb-0 small">
            ...
            @else
                Showing <strong>{{ $tasks->firstItem() }}-{{ $tasks->lastItem() }}</strong> of <strong>{{ $tasks->total() }}</strong> tasks.
            @endif
        </p>
        <div>
    ...
    <div id="bulkActionBar" class="bulk-action-bar shadow-lg border d-none">
        <div class="d-flex align-items-center justify-content-between px-3 py-2">
            ...
            <div class="d-flex gap-2">
                @if($category === 'trash')
                    <button type="button" class="btn btn-sm btn-light fw-bold" onclick="submitBulk('{{ route('tasks.bulkRestore') }}')">
                        <i class="bi bi-arrow-counterclockwise me-1"></i>Restore
                    </button>
                    <button type="button" class="btn btn-sm btn-danger fw-bold" onclick="submitBulk('{{ route('tasks.bulkForceDelete') }}', true)">
                        <i class="bi bi-trash3 me-1"></i>Delete Permanent
                    </button>
                @else
                    <button type="button" class="btn btn-sm btn-danger fw-bold" onclick="submitBulk('{{ route('tasks.bulkDestroy') }}')">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    window.submitBulk = function(url, confirmFirst = false) {
        if (confirmFirst && !confirm('Are you sure you want to permanently delete these items?')) {
            return;
        }
